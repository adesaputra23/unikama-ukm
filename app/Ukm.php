...
class Ukm extends Model
{
    ...
    public static function GetMaxMin($data)
    {
        ...
        if ($data->jenis_kriteria == 'Benefit') {
            $nilai = 'Max ('.Penilaian::where('npm', $user)->where('kode_kriteria', $data->kode_kriteria)->max('nilai').')';
        }else{
            $nilai = 'Min ('.Penilaian::where('npm', $user)->where('kode_kriteria', $data->kode_kriteria)->min('nilai').')';
        }
        return $nilai;
    }
